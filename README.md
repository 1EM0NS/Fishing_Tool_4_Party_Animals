...
⚡ **性能优势**：
本程序的钓鱼效率是人类操作的两倍！在游戏中，拉杆时会遇到两种张力情况：一种是张力几乎不变，另一种是张力急剧变化。程序能够快速响应这些变化，实时刷新张力情况，确保最佳的拉杆操作。
但是鱼满了或饵料没了需要自行卖出或购买，所以**Larger Fish Container is All You Need**
...
你可以在 UI 中点击 **“设置指针区域”** 或 **“设置咬钩区域”** 手动框选（使用回车键确认框选的区域，该区域需要尽量包含检测要素但不要过多背景）。
...
程序依赖 2 张模板图片，需放在脚本同目录下（项目里已经给出了）：
...
4. **设置区域后点击开始检测没反应？**
   ...
   * 请增加检测区域大小，区域小于模板会导致失败的检测条件。
5.**点击开始检测不自动钓鱼？**
   * 开始检测和钓鱼是两套独立功能，互不相关，即使没有开始检测，自动钓鱼逻辑也可自行运行（即使界面没有实时监测画面）。
5.**怎么钓着钓着人向右转了？**
   * 最好一开始站在河旁面朝左一点，游戏在钓鱼结束后视角会自动向右转动一点点，后续有空会修复，或者大家可以直接修改程序。
